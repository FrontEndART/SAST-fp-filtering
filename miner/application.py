...
def load_message_classifier():
    print("Loading message classifier...", end="")

    model = RobertaForSequenceClassification.from_pretrained('roberta-base', num_labels=2)
    model.to(device)
    model.load_state_dict(torch.load(message_model_path,map_location=torch.device('cpu')))
    model.eval()
...
def load_issue_classifier():
    print("Loading issue classifier...", end="")

    model = RobertaForSequenceClassification.from_pretrained('roberta-base', num_labels=2)
    model.to(device)
    model.load_state_dict(torch.load(issue_model_path,map_location=torch.device('cpu')))
    model.eval()
...
def load_patch_classifier():
    print('Loading patch classifier...', end="")
    ...
    finetune_model.load_state_dict(torch.load(patch_finetune_model_path,map_location=torch.device('cpu')))
    ...
    model.module.load_state_dict(torch.load(patch_model_path,map_location=torch.device('cpu')))
    model.to(device)
    model.eval()
...
def batch_predict(args):
    ...
    id_list = []
    for item in alive_it(item_list, bar="classic"):
        id = item['id']
        id_list.append(id)
        if 'message' in item:
    ...
    if MODE == 'prediction':
        ...
        id_prob_list.sort(key=lambda x: x[1], reverse=True)
